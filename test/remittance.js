...
const { toWei, toBN, sha3 } = web3.utils;
...
contract("Remittance", (accounts) => {
    ...
    it("Remit should not work if you send no value.", async function(){
        await truffleAssert.reverts(
            contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: alice, value: 0 }), 
    ...
    it("Only the owner can call remit.", async function(){
        ...
        await truffleAssert.reverts(
            contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: notOwner, value: amount }), 
            "Only the owner can call this function.")
    });

    it("Remit emits the proper event", async function(){
        const amount = toWei(toBN("1"), "Gwei");
        const remitObj = await contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: alice, value: amount});
        const { logs } = remitObj;
    ...
    it("Carols internal balance after remit should be right.", async function() {
        const amount = toWei(toBN("1"), "Gwei");
        await contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: alice, value: amount});
        const balanceAfter = await contractInstance.balances(carol);
    ...
    it("Carols internal balance after withdraw should be 0.", async function(){
        ...
        const carolsBalance = await contractInstance.balances(carol);
    ...
    it("Carols balance should not be reset after each remit.", async function(){
        ...
        await contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: alice, value: amount1 });
        await contractInstance.remit(carol, sha3("0x123"), sha3("0x456"), { from: alice, value: amount2 });
        const carolsBalance = await contractInstance.balances(carol);
